...
- R6.m A balance between budgets MAY be closed by agreement — *call it even* — in whole or in part, by whoever is ahead. It is an expense filed to a category in their budget and income in the other's (`15` §4A.4), never a number deleted. The expense MUST then be funded like any other, from the forgiving budget's Ready to Assign — that is where the money comes from, and leaving it unfunded is an ordinary overspend carried into next month. It is never a reason the household pays the bank less. Covering your *own* negative household envelope is not this: it raises your share for the month (`15` §4A.2) and is the existing cover-overspend flow.
- R6.n Copy about money between household members MUST NOT use the language of debt (`15` §4A.1). *Owes*, *debt*, *write off* and *forgive* are right for family lending, where the other party is outside the household, and wrong between partners. Offer *put it down to me*, *I'll pick it up*, or *call it even*.
- R6.n.1 **Superseding R6.n's *ahead* and *behind*.** Those words were built and then used, and they describe *a person* — so the same balance read "₹36,640 ahead" in one table and "the household is ₹36,640 behind with Ravi" two inches below, which is one fact stated from two subjects and read as a bug. Every sentence about a balance MUST have **the commitment** as its subject, and MUST use the budget screen's own words for an envelope holding too little or too much: **underfunded** and **overfunded**. A test asserts the subject and forbids both of the old words.
- R6.n.2 A sentence about a commitment MUST say **both** where the money came from and what it paid for. "Gone to the household" reads as cash handed over and "gone from the household" as the household paying; what happened is that *the household's spending was paid out of one person's money*, and only saying both halves is unambiguous.
- R6.s The envelope a called-even amount is spent from MUST be **named before the act**, and the giving budget's other envelopes offered. `15` §4A.5 proposed *Gifts and treats* as the default; it is the wrong word more often than the right one, because agreeing to leave a lopsided month is usually not a present and naming it one attributes a generosity neither person claimed. The default is **Settled between us**.
- R6.t Both sides of calling it even MUST be stated where it is done: an expense for the one giving it up, and **income for the one receiving it** (`15` §4A.5). Saying only the first makes it read as money disappearing. The receiving side is not always the envelope's own budget, and reading it off the envelope left the other budget's books short by the amount let go in every month from then on.
- R6.u Where a balance can be settled, only the option that is **the reader's to take** is offered. Funding your own commitment and picking up somebody else's are the same amount of money and opposite acts; showing both and greying one asks the reader to work out which is theirs. All the endings MUST accept an amount, because all of them can be partial.
- R6.k An **add-on** card's spending is liable to the primary holder, so the payment envelope stays in the account's budget whoever made the charge (`15` §3A.5). Filing an add-on charge to the holder's own envelope MUST raise a claim from the holder to the primary; filing it to a household envelope raises one from the household to the primary.
- R6.l A claim between two **personal** budgets is permitted only where a shared instrument links them — an add-on card, or an account in the household budget. There is no other way for one member to put a charge on another's budget.
...
- R6.o Where a transaction's account is in one budget and its category in another, the **claim between the two budgets** MUST absorb exactly the difference, and the envelope carrying it MUST exist before the transaction does (`15` §3A.4). Activity belongs to the category's budget and the cash to the account's; attributing both to the account's left the receiving budget's envelope untouched and broke both identities in opposite directions.
- R6.p A **commitment envelope's negative MUST NOT be absorbed** at a rollover (`15` §6.1). An overspent envelope reopens at zero and charges Ready to Assign; a commitment in the red is a standing balance between two budgets, and absorbing it made the receiving budget's claim vanish while the payer's own pool took the hit.
- R6.q A transfer leg is internal — and so outside Ready to Assign — only when its other leg is in the **same** budget, or is a credit account (`15` §3.5). Cash moved into another budget's account has genuinely left and creates no claim; money paid toward another budget's card buys a claim, because nothing arrived anywhere that budget owns.
- R6.r A **month close is per budget** (`15` §6.1). The household's month MAY close while a personal one is open, and the household's close reports what each member committed and spent.
- R6.w Converting a card purchase to EMI MUST be modelled as **a loan whose money went to the card** (`06` §7.4). Crediting the card is what makes its outstanding fall and its payment envelope stop asking for money that is now an instalment plan — the double-funding §7.4 singles out. The processing fee and its GST MUST be charged to the card and given an envelope, because they are a real purchase as well as a cost of borrowing (R22). The loan MUST record which card the purchase was made on and which charge it came from.
- R6.v A card's **statement day** MUST tag each charge with the statement cycle it bills in, on the account register. A cycle runs from the day after one statement date up to and including the next, so a charge on the 19th with a statement day of 18 bills the following month — a boundary that decides whether a cycle reconciles against the paper statement. It clamps in a short month the way a due date does. Before this the column was recorded, editable and read by nothing.
- R6.x A screen that **prices a choice MUST carry it out** — and the tenure is what makes that possible. Two of them were decorative: a rate reset showed *keep the instalment* against *keep the tenure*, priced both, and always did the second; a prepayment offered *reduce the tenure* against *reduce the EMI*, recommended the first, and always did the second. The projection derives the instalment from what is outstanding over what is left of the tenure, so the tenure is the only lever that holds an instalment still. It is now live, and the tenure a loan was **first** scheduled over is recorded separately, because moving both together makes `06` R22's "instalments saved" read zero for ever.
- R6.y The options on a rate-change screen MUST be **recomputed at the rate being typed**. They were priced off a query parameter nothing on the page could set, so entering the new rate in the form left the table answering a question about the old one — the two figures the decision turns on, both wrong, on the screen where the decision is made.
- R6.z A prepayment's **funding envelope MUST actually fund it** (`06` R19.4). The field existed, said in as many words that it was there so no envelope would be quietly drained, and was read by nothing: a lakh left through the loan's own envelope, which holds one instalment, and reported an overspend against a decision the household had made deliberately.
- R6.aa A **categorised transaction on a tracking account is an attribution, not a payment**, and the engine MUST treat it as one. Money crossed the budget's edge when it reached the tracking account — lending ₹10,000 to a cousin reduced Ready to Assign the day the transfer happened, with no envelope named. Filing a category against that account later says *which envelope it had been*, so the envelope takes the amount and Ready to Assign gives it back. Counting only the envelope half broke the identity by the amount attributed, from that month forward, in every budget: writing off a family loan did it, and so did filing a category against a tracking account by hand.
- R6.ab A **loan instalment MUST be filed to the loan's own payment envelope**, not posted as a bare transfer to its tracking account (`06` R14: "symmetric with a card's"). A transfer to a tracking account carries no envelope, so the money left through Ready to Assign while the envelope kept everything ever assigned to it — a household that budgeted for its EMI paid for it twice in the budget's terms, and every loan read "not funded" however faithfully the instalments were paid.
- R6.ac A screen that **filters what it offers MUST filter what it accepts**. Closing the read side of per-member privacy left every form field taking an id by hand: posting another member's envelope id filed a household transaction into their private envelope, moved the household's money into it, and confirmed the envelope existed by succeeding. Both halves, or neither.
- R6.ad **Reports and the ledger query MUST take the viewer**, the same as the account list and net worth. `queryTransactions` is what Query, its CSV export, Search and the whole reports page are built on, and it read straight past the visibility flag: one member's private spending appeared to the rest of the household line by line, payee and amount and envelope.
