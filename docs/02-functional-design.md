...
- H2.2 A **Tracking** account — an asset, a loan, family lending, a deposit — MAY be **private**, visible only to its holder. A Budget or Credit account MUST NOT be, and the reason is arithmetic rather than policy: Ready to Assign is a sum over every Budget account, so hiding one while showing the total publishes it by subtraction. The app refuses the combination rather than offering a promise it cannot keep.
- H2.2a The prohibition in H2.2 holds while every account is in the one household budget. Once personal budgets exist (`15` §3.3) it MUST be relaxed to permit a private Budget or Credit account **when it belongs to a personal budget** — there the household never sums the account, only the amount its owner has committed, so nothing is published by subtraction. Relaxing it before then would reintroduce the leak.
- H2.3 A private account MUST be absent from other members' **totals** as well as their lists — net worth, allocation, reports. A line hidden but still summed is published to anyone who can see every other line.
- H2.4 Privacy MUST be resolved against the **authenticated** member, not the member being viewed as (R38.6). Otherwise "view as" is a way to read exactly what somebody marked private.
