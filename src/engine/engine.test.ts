...
describe("R9 · Auto-assign", () => {
  function context(scenario: Scenario, month = AUG) {
    const input = scenario.build();
    const state = computeBudget(input).get(month)!;
    return {
      month,
      ...
      historicalAssigned: new Map(
        input.months.map((m) => [m, input.facts[m]?.assigned ?? {}]),
      ),
      today: "2026-08-01",
    };
  }

  test("fills the month and previews the resulting RTA before committing", () => {
    const ctx = context(
      new Scenario().category("rent").category("groceries").income(AUG, 100_000),
    );

    const plan = planAutoAssign(
      [
        { categoryId: "rent", type: "fixed", priority: 1, amount: rupees(35_000) },
        { categoryId: "groceries", type: "fixed", priority: 1, amount: rupees(15_000) },
      ],
      ctx,
    );

    assert.equal(plan.totalAssigned, rupees(50_000));
    assert.equal(plan.rtaBefore, rupees(100_000));
    assert.equal(plan.rtaAfter, rupees(50_000));
    assert.equal(plan.proposals.length, 2);
  });

  test("spends only money actually held and stops cleanly (R1)", () => {
    const ctx = context(new Scenario().category("rent").category("groceries").income(AUG, 40_000));

    const plan = planAutoAssign(
      [
        { categoryId: "rent", type: "fixed", priority: 1, amount: rupees(35_000) },
        { categoryId: "groceries", type: "fixed", priority: 2, amount: rupees(15_000) },
      ],
      ctx,
    );

    assert.equal(plan.rtaAfter, 0, "never drives RTA negative");
    assert.equal(plan.proposals[0]!.delta, rupees(35_000));
    assert.equal(plan.proposals[1]!.delta, rupees(5_000));
    assert.equal(plan.proposals[1]!.limitedByAvailableFunds, true);
  });

  test("runs band by band, so a lower band never starves a higher one", () => {
    const ctx = context(
      new Scenario().category("wants").category("rent").income(AUG, 30_000),
    );

    const plan = planAutoAssign(
      [
        { categoryId: "wants", type: "fixed", priority: 9, amount: rupees(20_000) },
        { categoryId: "rent", type: "fixed", priority: 1, amount: rupees(25_000) },
      ],
      ctx,
    );

    assert.equal(plan.proposals[0]!.categoryId, "rent", "band 1 goes first");
    assert.equal(plan.proposals[0]!.delta, rupees(25_000));
    assert.equal(plan.proposals[1]!.delta, rupees(5_000));
  });

  test("refill tops up to the target balance rather than adding to it", () => {
    const ctx = context(
      new Scenario()
        .category("car-service")
        .income("2026-07", 50_000)
        .assign("2026-07", "car-service", 6_000)
        .income(AUG, 50_000),
    );

    const plan = planAutoAssign(
      [{ categoryId: "car-service", type: "refill", priority: 1, amount: rupees(10_000) }],
      ctx,
    );
    assert.equal(plan.proposals[0]!.delta, rupees(4_000));
  });

  test("a ceiling stops a fixed rule once the category is full", () => {
    const ctx = context(
      new Scenario()
        .category("emergency")
        .income("2026-07", 50_000)
        .assign("2026-07", "emergency", 19_000)
        .income(AUG, 50_000),
    );
      ...
      [
        {
          ...
          priority: 1,
          amount: rupees(4_000),
          ceiling: rupees(20_000),
        },
      ],
      ctx,
    );
    assert.equal(plan.proposals[0]!.delta, rupees(1_000), "only the headroom, not the full ₹4,000");
  });

  test("percent of income computes from income actually received (P1)", () => {
    const ctx = context(new Scenario().category("savings").income(AUG, 80_000));
    const plan = planAutoAssign(
      [{ categoryId: "savings", type: "percent-income", priority: 1, percent: 20 }],
      ctx,
    );
    assert.equal(plan.proposals[0]!.delta, rupees(16_000));
  });

  test("average of history uses the preceding months", () => {
    const ctx = context(
      new Scenario()
        .category("groceries")
        .income("2026-05", 50_000)
        .assign("2026-05", "groceries", 9_000)
        .income("2026-06", 50_000)
        .assign("2026-06", "groceries", 12_000)
        .income("2026-07", 50_000)
        .assign("2026-07", "groceries", 15_000)
        .income(AUG, 50_000),
    );

    const plan = planAutoAssign(
      [{ categoryId: "groceries", type: "average-history", priority: 1, months: 3 }],
      ctx,
    );
    assert.equal(plan.proposals[0]!.delta, rupees(12_000));
  });

  test("remainder sweeps run last and split what is left by weight", () => {
    const ctx = context(
      new Scenario().category("rent").category("savings").category("fun").income(AUG, 50_000),
    );

    const plan = planAutoAssign(
      [
        { categoryId: "savings", type: "remainder-sweep", priority: 1, weight: 3 },
        { categoryId: "fun", type: "remainder-sweep", priority: 1, weight: 1 },
        { categoryId: "rent", type: "fixed", priority: 9, amount: rupees(30_000) },
      ],
      ctx,
    );

    // Rent is band 9 but still runs before the sweeps, which always go last.
    assert.equal(plan.proposals[0]!.categoryId, "rent");
    assert.equal(plan.proposals[1]!.delta, rupees(15_000));
    assert.equal(plan.proposals[2]!.delta, rupees(5_000));
    assert.equal(plan.rtaAfter, 0, "the sweep takes RTA to exactly zero");
  });

  test("skips hidden categories (F3.2)", () => {
    const ctx = context(
      new Scenario()
        .category("rent")
        .category("old-thing", { hidden: true })
        .income(AUG, 50_000),
    );

    const plan = planAutoAssign(
      [
        { categoryId: "rent", type: "fixed", priority: 1, amount: rupees(10_000) },
        { categoryId: "old-thing", type: "fixed", priority: 1, amount: rupees(5_000) },
      ],
      ctx,
    );
  ...
  test("proposes nothing when there is no money", () => {
    const ctx = context(new Scenario().category("rent").income(AUG, 0));
    const plan = planAutoAssign(
      [{ categoryId: "rent", type: "fixed", priority: 1, amount: rupees(10_000) }],
      ctx,
    );
    assert.deepEqual(plan.proposals, []);
    assert.equal(plan.rtaAfter, 0);
  });

  test("describes each rule in plain language, never a syntax (Q9)", () => {
    assert.equal(
      describeAutoAssignRule(
        { categoryId: "c", type: "fixed-ceiling", priority: 1, amount: rupees(4_000), ceiling: rupees(20_000) },
        formatPaise,
      ),
      "Assign ₹4,000 every month, stopping when this category holds ₹20,000",
    );
    assert.equal(
      describeAutoAssignRule({ categoryId: "c", type: "percent-income", priority: 1, percent: 20 }, formatPaise),
      "Assign 20% of the income received this month",
    );
  });
});
